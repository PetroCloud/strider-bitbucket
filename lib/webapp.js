...
module.exports = {
  ...
  routes: function (app, context) {
    ...
    app.anon.post('commit/:secret', function (req, res) {
      ...
      api.startCommitJob(client, data, req.project, context.emitter, function (err) {
    ...
    app.anon.post('pull-request/:secret', function (req, res) {
      ...
      if (data.pullrequest_created) {
          api.startPRJob(client, data.pullrequest_created, req.project, context.emitter, function (err) {
              if (err) return res.status(500).send('Failed to start job');
              res.sendStatus(204);
          });
      }
      else if (data.pullrequest_updated) {
          api.startPRJob(client, data.pullrequest_updated, req.project, context.emitter, function (err) {
              if (err) return res.status(500).send('Failed to start job');
              res.sendStatus(204);
          });
      }
...
function listRepos(client, next) {
  ...
  client.get(url, function (err, data, req) {
    ...
    next(null,
      data.map(api.parseRepo).filter(function (repo) {
